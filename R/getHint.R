getHint <- function(pathDF, effect){
  ...
  nOpenNonCausal <- pathDF %>% dplyr::filter(open==TRUE & directed==FALSE) %>% nrow()
  nOpenCausal <- pathDF %>% dplyr::filter(open==TRUE & directed==TRUE) %>% nrow()
  nClosedCausal <- pathDF %>% dplyr::filter(open==FALSE & directed==TRUE) %>% nrow()
